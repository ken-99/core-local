...
- [~] Phase 2+3: Act 2 — the living shoreline.
  ...
  - [~] Act 2 foreshore reveal — CODE DONE, browser check pending (auth-gated, Jon).
    - Water is now terrain-derived: `Act2Water` samples the maptiler DEM once
      (`queryTerrainElevation`) over `WATER_SAMPLE`, and `buildWaterMesh` (act2.ts,
      unit-tested) clips it to the part below the current tide height. The waterline
      recedes across the foreshore instead of only rising in place; `SEA_POLYGON`
      is now the inland safety mask. Dropped `WATER_FLOOR_LEVEL_M` (low water is meant
      to retreat now). Known: the coarse ~30 m DEM makes the flat harbour basin fill/
      empty more abruptly than it creeps; parent spec Step 3 (GPU depth-occlusion) is
      the reserved escalation. Spec: `cdt-kp1-claude/specs/2026-07-06-roscoff-act2-foreshore-reveal-{design,plan}.md`.
